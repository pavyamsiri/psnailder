...
DEFAULT_PARAM_BOUNDS: Final[tuple[ParamBounds, ...]] = (
    ParamBounds(name="alpha", lo=0.0, hi=1.0),
    ParamBounds(name="b", lo=0.005, hi=0.1),
    ParamBounds(name="c", lo=0.0, hi=0.004),
    ParamBounds(name="theta0", lo=-np.pi, hi=np.pi),
    ParamBounds(name="scale_factor", lo=30.0, hi=70.0),
    ParamBounds(name="rho", lo=0.0, hi=0.18),
)
NUM_PARAMETERS: Final[int] = 6
...
type SmoothingFunc = Callable[[onp.Array2D[np.float64]], onp.Array2D[np.float64]]
type ParamName = Literal["alpha", "b", "c", "theta0", "scale_factor", "rho"]
...
class AlinderModelCollection:
    ...
    def __post_init__(self) -> None:
        assert self.parameters.shape[1] == NUM_PARAMETERS
...
class SpiralFitter:
    def __init__(
        ...
        *,
        ...
        smoothing_func: SmoothingFunc | None = None,
        param_lo: dict[ParamName, float] | None = None,
        param_hi: dict[ParamName, float] | None = None,
    ) -> None:
        self._num_samples: int = num_samples
        self._num_discard: int = num_discard
        self._num_walkers: int = num_walkers
        self._max_iterations: int | None = max_iterations
        self._smoothing_func: SmoothingFunc = SpiralFitter._default_smoothing if smoothing_func is None else smoothing_func
        self._param_lo: onp.Array1D[np.float64] = np.array([param.lo for param in DEFAULT_PARAM_BOUNDS], dtype=np.float64)
        self._param_hi: onp.Array1D[np.float64] = np.array([param.hi for param in DEFAULT_PARAM_BOUNDS], dtype=np.float64)

        default_lo: dict[ParamName, float] = {p.name: p.lo for p in DEFAULT_PARAM_BOUNDS}
        default_hi: dict[ParamName, float] = {p.name: p.hi for p in DEFAULT_PARAM_BOUNDS}
        ...
        self._param_lo = np.array([default_lo[p.name] for p in DEFAULT_PARAM_BOUNDS], dtype=np.float64)
        self._param_hi = np.array([default_hi[p.name] for p in DEFAULT_PARAM_BOUNDS], dtype=np.float64)
    ...
    def fit_spiral(
        ...
        z_mesh, vz_mesh = np.meshgrid(z_centres, vz_centres, indexing="ij")
        ...
        background = estimated_background / estimated_background.sum() * density_total
    ...
    def fit_spiral_with_background(
        ...
        while self._max_iterations is None or (num_iterations < self._max_iterations):
            ...
            p0 = rng.uniform(self._param_lo, self._param_hi, size=(self._num_walkers, NUM_PARAMETERS))
