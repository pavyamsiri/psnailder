...
def test_broadcast_free_parameters_remain_independent() -> None:
    ...
    np.testing.assert_array_equal(optimizer.call_args.kwargs["bounds"].lb, [0.1, 0.1])
    np.testing.assert_array_equal(optimizer.call_args.kwargs["bounds"].ub, [0.9, 0.9])
    np.testing.assert_array_equal(optimizer.call_args.kwargs["x0"], [0.2, 0.7])
    assert isinstance(outcome, FitSuccess)
...
def test_layout_preserves_component_order_and_fixed_values() -> None:
    layout = _ParameterLayout.from_bounds([arm(Interval(0.1, 0.4)), arm(Interval(0.5, 0.9), np.pi)])
...
def test_reduced_guess_and_expanded_result() -> None:
    ...
    np.testing.assert_array_equal(de.call_args.kwargs["x0"], [0.2, 0.7])
    np.testing.assert_array_equal(de.call_args.kwargs["bounds"].lb, [0.1, 0.5])
    assert isinstance(outcome, FitSuccess)
    np.testing.assert_array_equal(outcome.result.final_model.to_array(), guess)
...
def test_bic_counts_fixed_parameters_as_zero() -> None:
    ...
    mask = fitter._mask_func(coordinates, coordinates)
    assert outcome.result.lnl == pytest.approx(ln_likelihood(data, outcome.result.final_model.prediction(), mask))
...
def test_layout_rejects_duplicate_indices_and_wrong_shapes() -> None:
    with pytest.raises(ValueError, match="unique"):
        _ParameterLayout(np.zeros(6), np.array([0, 0], dtype=np.intp), np.zeros(2), np.ones(2))
    layout = _ParameterLayout.from_bounds([arm(Interval(0.0, 1.0))])
    with pytest.raises(ValueError):
        layout.pack(np.zeros((6, 1)))
    with pytest.raises(ValueError):
        layout.unpack(np.zeros((1, 1)))
